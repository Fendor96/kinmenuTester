...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayResults(restaurants) {
        ...
        restaurants.forEach(restaurant => {
            ...
            li.innerHTML = `
                <h3>${restaurant.name || restaurant.nom}</h3>
                <p><strong>Commune:</strong> ${restaurant.commune || "Non spécifiée"}</p>
                <p><strong>Type:</strong> ${restaurant.type_restaurant || restaurant.type}</p>
                ${restaurant.rank_type ? `<p><strong>Rang:</strong> ${restaurant.rank_type}</p>` : ''}
                <p><strong>Prix Moyen par personne:</strong> ${restaurant.average_price_per_person || restaurant.prixMoyen} USD</p>
                <p>${(restaurant.is_datable === 1 || restaurant.isDatable === "oui") ? '✨ Idéal pour un date ! ✨' : 'Convient à d\'autres occasions.'}</p>
        ...
        searchResultsDiv.classList.add('flex', 'next');
    ...
    const updateSlider = () => {
        ...
        bestOfContainer.innerHTML = `
            <div class="slides">
                ...
                <a class="links" id="link" href="${currentBestOf.link}">${currentBestOf.link || ''}</a>
            </div>
        `;
